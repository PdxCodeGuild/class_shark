let app = new Vue({
  ...
  methods: {
    hacking: function(hax){
      for (let i = 0; i < hax; i++) {
        this.hackStuff += this.allTheHax.shift() + ' ';
      }
    ...
    doTheHack: function() {
      ...
      for (let i = 0; i < 100; i++) {
        ...
        if (i > 10) {
          ...
          this.hackStuff += percent
        } else if (i < 10) {
          this.hackStuff = this.hackStuff.slice(0,-1)
          this.hackStuff += percent
        } 
      }
      this.hackStuff += ` Done!\n`
  ...
  mounted() {
    this.allTheHax = this.allTheHax.split('');
    window.addEventListener("keypress", function(e) {
      let k = e.keyCode
      if (k === 32) {
        app.hacking(9*app.mult);
      } else if (k === 9) {
