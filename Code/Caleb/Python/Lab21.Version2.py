...
print('Welcome to Version 2 of Lab 21 - Count Words. A program written in python by Caleb Mealey')

def openfile(file_location):
    print(f'The 10 most reoccuring combination of unique words found in {file_location} outside of the standard common words listed in the "stop words" list are:')
    with open(f'{file_location}', 'r', encoding="utf8") as b:
        ...
        for i in range(len(book_text_no_punc_sw_ls)-1):
            word_combo = book_text_no_punc_sw_ls[i] + ' ' + book_text_no_punc_sw_ls[i+1]
            word_dict[word_combo] = word_dict.get(word_combo, 0) + 1
